	...
	// khai bao chan dieu khien step driver 

	const int J1stepPin = 2; //const int is read-only  //int read and write
...
//limit switch for calib  
	...
	void setup() {
	  ...
	  pinMode(J1stepPin, OUTPUT); // pin 2-13
	  ...
	  pinMode(J1calPin, INPUT); //pin 14 -19 
	...
	void loop() {
	  ...
	  while (Serial.available() > 0)// đọc dữ liệu   
	  {
	    char recieved = Serial.read(); 
	    inData += recieved;
	
            // ký tự xuống dòng thì xử lý lệnh
	    if (recieved == '\n')
	    {
	      String function = inData.substring(0, 2); // nhận string 0 đến 2 từ inData
